    ...
    $.post(url, {id: item_id, post_data: post_data, type: type, shopp: shopp}, function (res) {
        if (res.status == 1) {
            if (res.info.fee || res.info.ratio) {
                var html = '<div class="layui-row float-box" style="font-size: 10px;line-height: 15px;height: 15px">佣金比率：' + res.info.ratio + '&nbsp;预估：' + res.info.fee + '&nbsp;月销量：' + res.info.month_sell_num + '</div>';
