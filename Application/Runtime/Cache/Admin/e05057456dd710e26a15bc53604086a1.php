...
<div class="wrapper" style="position: relative;margin-top:70px;margin-bottom: 50px">
    ...
    <?php if(($group_id) == "1"): ?><form class="layui-form xbs" action="<?php echo U('TopLine/newsList');?>">
    ...
    <div class="layui-input-inline" style="width: 800px">
        <input type="text" name="title" placeholder="请输入文章标题" autocomplete="off"
               class="layui-input" value="<?php echo I('get.title');?>" id="title">
    </div>
    <div class="layui-input-inline">
    ...
    <table class="layui-table">
        ...
        <?php if(is_array($data)): $i = 0; $__LIST__ = $data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
                <td>
                    <?php if(($vo['is_send'] == 0) AND ($vo['is_save'] == 0)): ?><a href="javascript:;" class="update-news-info" data-id="<?php echo ($vo["id"]); ?>" data-title="<?php echo ($vo["title"]); ?>"
                        ...
                        <a href="javascript:;" class="news-info"
                           data-url="<?php echo U('TopLine/newsInfo',array('id'=>$vo['id']));?>"
                           style="color: #fff"
                           data-title="预览文章"
                    ...
                    <div class="layui-btn layui-btn-xs check-title"
                         data-title="<?php echo ($vo["title"]); ?>" data-type="2">检测
                    </div>
                </td>
                <td>
                ...
                <td>
                    <?php $save_type = array('item'=>'商品','img'=>'图片','img_desc'=>'图文') ?>
                    <?php if($vo['is_send'] == 0): if(($vo['is_save']) == "1"): ?><a class="layui-btn layui-btn-sm layui-disabled"><?php echo ($save_type[$vo['save_type']]); ?>存稿成功</a>
                            <?php else: ?>
                            <?php if(is_array($save_type)): foreach($save_type as $k=>$row): ?><a class="layui-btn layui-btn-sm figure"
                                   data-url="<?php echo U('TopLine/figure',array('id'=>$vo['id'],'type'=>$k));?>"
                                   href="javascript:;">存<?php echo ($row); ?></a><?php endforeach; endif; endif; endif; ?>
                    <!--<?php if($vo['is_save'] == 0): if(($vo['is_send']) == "1"): ?><a class="layui-btn layui-btn-sm layui-btn-normal layui-disabled"
                               href="javascript:;">已发表</a>
                            <?php else: ?>
                            <a class="layui-btn layui-btn-sm layui-btn-normal publish" href="javascript:;"
                               data-url="<?php echo U('TopLine/publish',array('id'=>$vo['id']));?>">发表</a><?php endif; endif; ?>-->
                    ...
                    <?php if(($vo['is_send'] == 0) AND ($vo['is_save'] == 0)): ?><a class="layui-btn layui-btn-sm layui-btn-danger news-del" href="javascript:;"
                           data-url="<?php echo U('TopLine/newsDel',array('id'=>$vo['id']));?>">删除</a><?php endif; ?>
                </td>
...
<!-- 背景切换结束 -->
    ...
    <script type="text/javascript">
        ...
        $('.check-title').click(function () {
            ...
            $.get("<?php echo U('checkTitle');?>", {title: title}, function (res) {
