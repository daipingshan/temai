...
<div class="wrapper" style="position: relative;margin-top:70px;margin-bottom: 50px">
    ...
    <form class="layui-form xbs" action="<?php echo U('SaleArticle/userArticle');?>" name="search">
        <div class="layui-form-pane">
            <div class="layui-form-item" style="display: inline-block;">
                <div class="layui-input-inline">
                    <select name="create_user_id">
                        <option value="0">请选择作者</option>
                        <?php if(is_array($author)): foreach($author as $k=>$row): ?><option value="<?php echo ($k); ?>"
                            <?php if(I('get.create_user_id') == $k): ?>selected<?php endif; ?>
        ...
        <?php if(is_array($data)): $i = 0; $__LIST__ = $data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
                ...
                <td>
                    <?php if($vo['article_genre'] == 1): ?><button class="layui-btn layui-btn-sm layui-btn-normal layui-disabled">图集</button>
                        <?php else: ?>
                        <button class="layui-btn layui-btn-danger layui-btn-sm layui-disabled">文章</button><?php endif; ?>
                </td>
                <td>
                    <?php echo (date("Y-m-d H:i:s",$vo["behot_time"])); ?>
                </td>
                <td>
                    <?php echo ($author[$vo['create_user_id']]); ?>【<?php echo ($vo["num"]); ?>】
                </td>
                <td>
