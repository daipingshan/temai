        ...
        <div class="item_content" id="imageChange">
            <ul>
                <?php if(is_array($data)): $i = 0; $__LIST__ = $data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><li>
                        <div class="item" data-id="<?php echo ($vo["shop_goods_id"]); ?>">
                            <img src="<?php echo ($vo["img"]); ?>">
                            <div class="text">
                                <?php echo ($vo["description"]); ?>--<?php echo ($vo["sec_cid"]); ?>
                            </div>
                            <span data-id="<?php echo ($vo["shop_goods_id"]); ?>" class="rmPicture close">×</span>
                        </div>
       ...
       */
        ...
        $('.item .close').click(function () {
            ...
            layer.confirm("你确定要删除该选品吗？", function () {
                $.post("<?php echo U('Sale/delItemCache');?>", {id: shop_goods_id}, function (res) {
                    layer.msg(res.info);
                    if (res.status == 1) {
