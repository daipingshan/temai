...
<div class="wrapper" style="position: relative;margin-top:70px;margin-bottom: 50px">
    ...
    <?php if(($group_id) == "1"): ?><form class="layui-form xbs" action="<?php echo U('TopLine/albumList');?>">
            <div class="layui-form-pane">
                <div class="layui-input-inline" style="width: 300px">
                    <input type="text" name="title" value="<?php echo I('get.title');?>" class="layui-input">
                </div>
                <div class="layui-input-inline">
                    <select name="is_send">
                        <option value="">请选择发文状态</option>
                        <option value="0" <?php if($is_send == 0): ?>selected<?php endif; ?>>未发送</option>
                        <option value="1" <?php if($is_send == 1): ?>selected<?php endif; ?>>已发送</option>
                    </select>
                </div>
    ...
    <table class="layui-table">
        ...
        <?php if(is_array($data)): $i = 0; $__LIST__ = $data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
                ...
                <td>
                    <?php echo ($vo["fail_reason"]); ?>
                </td>
                <td>
                    <?php if($vo['is_send'] == 0): ?><button type="button" class="layui-btn layui-btn-xs layui-btn-warm figure"
                                data-url="<?php echo U('TopLine/albumFigure',array('id'=>$vo['id']));?>">立即存搞
                        </button>
                        <?php else: ?>
