        ...
        <div class="layui-row">
            <?php if(is_array($data)): $i = 0; $__LIST__ = $data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><div class="layui-row">
                    <?php if(is_array($vo)): $i = 0; $__LIST__ = $vo;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$row): $mod = ($i % 2 );++$i;?><div class="layui-col-xs12 layui-col-sm6 layui-col-md3">
                            <div class="item-box">
                                <div class="img">
                                    <img lay-src="<?php echo ($row["pic"]); ?>">
                                    <div class="intro">
                                        <p><?php echo ($row["desc"]); ?></p>
                                    </div>
                                </div>
                                <div class="item-content">
                                    <a href="<?php echo ($row["coupon_url"]); ?>" target="_blank"><p class="item-title">
                                        <?php echo ($row["short_title"]); ?></p></a>
                                    <a href="<?php echo ($row["coupon_url"]); ?>" target="_blank">
                                        <p class="article-title"><?php echo ($row["title"]); ?></p>
                                    </a>
                                    ...
                                    <div class="layui-row float-box">
                                        ...
                                        <div class="layui-col-xs4">
                                            <div class="time">
                                                <b><?php echo ($row["commission_rate"]); ?>%</b>
                                            </div>
                                        </div>
                                    </div>
                                    <div class="layui-row float-box">
                                        <div class="layui-col-xs6">
                                            <?php if(($row['is_send']) == "Y"): ?><button class="layui-btn">已发送微头条</button>
                                                <?php else: ?>
                                                <button class="layui-btn default-bgcolor sendItem"
                                                        data-id="<?php echo ($row["id"]); ?>" data-type="wei_top">
                                                    微头条
                                                </button><?php endif; ?>
                                        </div>
                                        <div class="layui-col-xs6">
                                            <?php if(($row['is_auto']) == "1"): ?><button class="layui-btn">已发送头条</button>
                                                <?php else: ?>
                                                <button class="layui-btn default-bgcolor sendItem"
                                                        data-id="<?php echo ($row["id"]); ?>" data-type="top">
                                                    头条
                                                </button><?php endif; ?>
                                        </div>
                                    </div>
...
<!-- 背景切换结束 -->
    ...
    <script type="text/javascript">
        ...
        $('.sendItem').click(function () {
            ...
            var url = "<?php echo U('TopLine/sendGoods');?>";
            is_add = false;
