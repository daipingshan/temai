...
<div class="wrapper" style="position: relative;margin-top:70px;margin-bottom: 50px">
    ...
    <table class="layui-table">
        ...
        <?php if(is_array($data)): $i = 0; $__LIST__ = $data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
                <td>
                    <?php if($vo['is_top'] == 1): ?><a class="layui-btn layui-btn-xs">头条</a>
                        <?php else: ?>
                        <a class="layui-btn layui-btn-xs layui-btn-danger">非头条</a><?php endif; ?>
                    <a href="<?php echo ($vo["article_url"]); ?>" target="_blank" style="color: #fff"><?php echo ($vo["title"]); ?></a>
                ...
                <td>
                    <a class="layui-btn layui-btn-sm" href="<?php echo U('taoJinGeArticleInfo',array('id'=>$vo['id']));?>"
                       target="_blank">查看详情</a>
                    <?php if(($vo["is_send"] == 0) AND ($vo["is_normal"] == 1)): ?><a class="layui-btn layui-btn-sm layui-btn-danger send-article"
                           data-url="<?php echo U('selectAccount',array('id'=>$vo['id']));?>">一键发文</a>
                        <?php else: ?>
                        <?php if($vo["is_send"] == 1): ?><a class="layui-btn layui-btn-sm">已发送</a><?php endif; ?>
                        <?php if($vo["is_normal"] == 0): ?><a class="layui-btn layui-btn-sm layui-btn-normal">等待数据加载</a><?php endif; endif; ?>
                </td>
...
<!-- 背景切换结束 -->
    ...
    <script>
        $('.check-title').click(function () {
            ...
            $.get("<?php echo U('TopLine/checkTitle');?>", {title: title}, function (res) {
        ...
        $('#save-news').click(function () {
            ...
            $.post("<?php echo U('sendArticle');?>", {id: id, account_id: account_id}, function (res) {
                layer.msg(res.info);
                if (res.status == 1) {
