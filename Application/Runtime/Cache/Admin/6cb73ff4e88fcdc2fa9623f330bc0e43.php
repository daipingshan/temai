...
<div class="wrapper" style="position: relative;margin-top:70px;margin-bottom: 50px">
    ...
    <form class="layui-form xbs" action="<?php echo U('TopLine/itemsList');?>" name="search">
        <div class="layui-form-pane">
            <div class="layui-form-item" style="display: inline-block;">
                <div class="layui-input-inline">
                    <button class="layui-btn" style="background-color: rgba(0,0,0,0.4)"
                            data-url="<?php echo U('Sale/selectItems',array('type'=>'top_line'));?>"
                            onclick="x_admin_show('选择商品',$(this).data('url'),'90%','96%')" type="button">选择商品
                    </button>
                </div>
                <div class="layui-input-inline">
        ...
        <?php if(is_array($vo)): $i = 0; $__LIST__ = $vo;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$row): $mod = ($i % 2 );++$i;?><div class="layui-col-xs12 layui-col-sm6 layui-col-md3">
                <div class="item-box">
                    <div class="img">
                        ...
                        <div class="intro">
                            <p><?php echo ($row["describe_info"]); ?></p>
                        </div>
                    </div>
                    <div class="item-content">
                        <a href="<?php echo ($row["url"]); ?>" target="_blank"><p class="item-title"><?php echo ($row["title"]); ?></p></a>
                        <?php if($row["temai_title"] != ''): ?><a href="<?php echo ($row["temai_id"]); ?>" target="_blank">
                                <p class="article-title"><?php echo ($row["temai_title"]); ?></p>
                            </a><?php endif; ?>
                        ...
                        <div class="layui-row float-box">
                            ...
                            <div class="layui-col-xs7">
                                <?php if($row["behot_time"] != ''): ?><div class="time">
                                        <?php echo (date("Y-m-d H:i",$row["behot_time"])); ?>
                                    </div><?php endif; ?>
                            </div>
                        </div>
                        ...
                        <div class="layui-row float-box">
                            <?php if(($type) == "goods"): ?><button class="layui-btn add-goods" data-title="添加机器商品"
                                        data-url="<?php echo U('addGoods',array('id'=>$row['id'],'type'=>$row['type']));?>">
                                    添加机器商品
                                </button>
                                <?php else: ?>
                                <?php if(($row["is_add"]) == "1"): ?><div class="layui-row add-box layui-hide">
                                        <div class="layui-col-xs6">
                                            <button class="layui-btn default-bgcolor addItemCache"
                                                    ...
                                                    data-url="<?php echo U('TopLine/addItemCache');?>"
                                    ...
                                    <div class="layui-row del-box">
                                        <button class="layui-btn layui-btn-danger del-item" data-id="<?php echo ($row["id"]); ?>"
                                                data-url="<?php echo U('TopLine/delItemCache');?>">删除选品
                                        </button>
                                    </div>
                                    <?php else: ?>
    ...
    <div class=" layui-col-xs2" style="text-align: center;line-height: 50px">
        <div style="position: relative;width: 50px;height: 40px;line-height: 40px;margin: 5px auto">
            <i class="layui-icon temai-preview" style="color: #fff;font-size: 30px;cursor: pointer"
               data-url="<?php echo U('cartSortList');?>" data-title="预览文章" data-width="85%" data-height="95%">&#xe653;</i>
        </div>
    </div>
    <div class=" layui-col-xs2" style="text-align: center;line-height: 50px">
...
<!-- 背景切换结束 -->
    ...
    <script type="text/javascript" src="/Public/Admin/js/top_line_item.js?v=3"></script>
    <script type="text/javascript">
        form.render();
        //日期时间范围
        laydate.render({
